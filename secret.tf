...
module "secret_context" {
  ...
  enabled    = module.context.enabled && var.create_vpn_secret
  attributes = ["secret"]
}
...
locals {
  secret_arn         = module.secret_context.enabled ? one(aws_secretsmanager_secret.this[*].arn) : var.vpn_secret_arn
  secret_kms_key_arn = module.secret_context.enabled ? module.secret_kms_key.key_arn : var.vpn_secret_kms_key_arn
}
...
module "secret_kms_key" {
  ...
  enable_key_rotation      = var.vpn_secret_enable_kms_key_rotation
  ...
  secret_string = jsonencode(merge(
    {
      ...
      "${var.vpn_secret_admin_password_key}" = one(random_password.admin[*].result)
    }
  ))
}
