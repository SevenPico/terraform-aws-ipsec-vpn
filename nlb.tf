...
locals {
  nlb_listener_protocols      = ["UDP", "UDP"]
  nlb_listener_ports          = [500, 4500]
  nlb_target_protocols        = ["UDP", "UDP"]
  nlb_target_ports            = [500, 4500]
  nlb_target_type             = ["instance", "instance"]

  nlb_target_groups = [for x in aws_lb_target_group.nlb[*] :
    {
      elb_name : null
